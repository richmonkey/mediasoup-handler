...
export class RecvTransport<TransportAppData extends AppData = AppData> extends Transport<TransportAppData> {
	...
	consume({
		...
		rtpParameters,
		streamId,
	}: ConsumerOptions): Promise<HandlerReceiveResult> {
	...
	async consumeData({
		...
		label = '',
		protocol = '',
	}: DataConsumerOptions): Promise<HandlerReceiveDataChannelResult> {
